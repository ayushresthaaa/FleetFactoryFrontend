...
import { getVehicles, searchVehicles } from "../../api/api";
...
export default function VehicleInfo() {
  ...
  const displayed = fuelFilter === "All"
    ? vehicles
    : vehicles.filter((v) => v.fuelType === fuelFilter);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {isSearchMode && (
        <div className="flex items-center gap-2 bg-[rgba(233,30,140,0.06)] border border-[rgba(233,30,140,0.15)] rounded-lg px-4 py-2.5">
          ...
          <span className="text-[#888] text-[12px]">
            Showing results for{" "}
            <span className="text-white font-semibold">"{searchQuery}"</span> — {vehicles.length} result(s)
          </span>
          <button
            onClick={clearSearch}
      ...
      <div className="flex gap-4 items-start">
        ...
        {/* Table */}
        <div className="flex-1 min-w-0 bg-[#1a1a1a] border border-[#252525] rounded-xl overflow-hidden">
          <div className="px-5 py-3.5 border-b border-[#252525] flex items-center justify-between">
            ...
            <span className="text-[#555] text-[12px]">
              {displayed.length} {isSearchMode ? "found" : "shown"}
            </span>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              <span className="material-icons text-[#333]" style={{ fontSize: "40px" }}>
                {isSearchMode ? "search_off" : "directions_car"}
              </span>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#252525]">
                    {["Vehicle", "Plate / VIN", "Owner", "Mileage", "Last Service", "Actions"].map((h) => (
                      <th key={h} className="text-left text-[#555] text-[11px] font-semibold uppercase tracking-wider px-5 py-3">
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {displayed.map((v) => {
                    ...
                    const isActive = selected && (selected.vehicleId ?? selected.id) === id;
                    const fc = fuelColor[v.fuelType] ?? { text: "#888", bg: "rgba(255,255,255,0.05)", border: "rgba(255,255,255,0.1)" };
                    const svc = svcStatus(v.lastService);
                    return (
                      <tr
                        key={id}
                        onClick={() => setSelected(isActive ? null : v)}
                        className={`border-b border-[#1f1f1f] transition-colors cursor-pointer
                          ${isActive ? "bg-[rgba(233,30,140,0.06)]" : "hover:bg-[#1f1f1f]"}`}
                      >
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1)
                  ...
                  .map((item, i) =>
                    item === "..." ? (
                      ...
                      <PageBtn key={item} active={item === pageNumber} onClick={() => setPageNumber(item)}>
                        {item}
                      </PageBtn>
                    )
                  )}
        ...
        {/* Detail panel */}
        {selected && (
          <div className="w-[272px] shrink-0 bg-[#1a1a1a] border border-[#252525] rounded-xl overflow-hidden">
            <div className="px-4 py-4 bg-[#141414] border-b border-[#252525] relative">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-[#252525] border border-[#333] flex items-center justify-center shrink-0">
                  <span className="material-icons text-[#555]" style={{ fontSize: "20px" }}>directions_car</span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-white text-[13px] font-semibold font-mono tracking-wide">{selected.vehicleNumber}</div>
                  <div className="text-[#555] text-[11px]">{selected.year} {selected.make} {selected.model}</div>
                </div>
              </div>
              <button
                onClick={() => setSelected(null)}
                className="absolute right-3 top-3 text-[#555] hover:text-white bg-transparent border-none cursor-pointer p-0"
              >
            ...
            <PanelSection title="Vehicle Details">
              <PanelRow label="Make"      value={selected.make ?? "—"} />
              <PanelRow label="Model"     value={selected.model ?? "—"} />
              <PanelRow label="Year"      value={selected.year ?? "—"} />
              <PanelRow label="Color"     value={selected.color ?? "—"} />
              <PanelRow label="Fuel Type" value={selected.fuelType ?? "—"} />
              <PanelRow label="Engine CC" value={selected.engineCC ? `${selected.engineCC}cc` : "—"} />
              <PanelRow label="Mileage"   value={selected.mileage ? `${selected.mileage?.toLocaleString()} km` : "—"} />
            </PanelSection>

            <PanelSection title="Registration">
              <PanelRow label="Number Plate" value={selected.vehicleNumber} mono />
              <PanelRow label="VIN / Chassis" value={selected.vin ?? "—"} mono />
            </PanelSection>

            <PanelSection title="Service">
              <PanelRow label="Last Service" value={selected.lastService ?? "Not recorded"} />
              <div className="flex items-center justify-between mt-1">
                <span className="text-[#555] text-[11px]">Status</span>
                <span className="text-[11px] font-semibold" style={{ color: svcStatus(selected.lastService).color }}>
                  {svcStatus(selected.lastService).label}
                </span>
              </div>
            </PanelSection>

            <PanelSection title="Owner">
              <PanelRow label="Customer" value={selected.customerName ?? "—"} />
              <PanelRow label="Phone"    value={selected.customerPhone ?? "—"} />
            </PanelSection>

            {selected.notes && (
              <PanelSection title="Notes">
                <p className="text-[#555] text-[12px] m-0 leading-relaxed">{selected.notes}</p>
              </PanelSection>
            )}

            <div className="px-4 py-3 flex gap-2">
              <button
                onClick={() => { setEditVehicle(selected); setModalOpen(true); }}
                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg bg-gradient-to-r from-[#e91e8c] to-[#c2185b] text-white text-[12px] font-semibold border-none cursor-pointer hover:opacity-90 transition-opacity"
              >
              ...
              <button
                ...
                className="flex items-center justify-center px-3 py-2 rounded-lg bg-[rgba(239,68,68,0.08)] text-red-400 border border-[rgba(239,68,68,0.15)] hover:bg-[rgba(239,68,68,0.15)] text-[12px] cursor-pointer transition-colors"
              >
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-[#1a1a1a] border border-[#252525] rounded-xl p-6 w-[360px] shadow-2xl">
            ...
            <p className="text-[#555] text-[13px] m-0 mb-5 leading-relaxed">
              Remove{" "}
              <span className="text-white font-semibold font-mono">{deleteConfirm.vehicleNumber}</span>{" "}
              ({deleteConfirm.year} {deleteConfirm.make} {deleteConfirm.model}) from the system? This cannot be undone.
            </p>
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => setDeleteConfirm(null)}
                className="px-4 py-2 rounded-lg bg-transparent border border-[#333] text-[#888] hover:text-white text-[13px] font-medium cursor-pointer transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={() => {
                  // TODO: call deleteVehicle API then refetch
                  setDeleteConfirm(null);
                  if (selected && (selected.vehicleId ?? selected.id) === (deleteConfirm.vehicleId ?? deleteConfirm.id)) {
                    setSelected(null);
                  }
                  refetch();
                }}
...
function PanelSection({ title, children }) {
  return (
    <div className="px-4 py-3 border-b border-[#252525]">
      <div className="text-[#e91e8c] text-[10px] font-bold uppercase tracking-wider mb-2">{title}</div>
      {children}
    </div>
  );
}
...
function PanelRow({ label, value, mono }) {
  return (
    <div className="flex items-start justify-between gap-2 mb-1.5 last:mb-0">
      <span className="text-[#555] text-[11px] shrink-0">{label}</span>
      <span className={`text-[11px] text-right break-all ${mono ? "text-[#ccc] font-mono" : "text-[#888]"}`}>{value}</span>
    </div>
  );
}
